...
wait_event:       
            ...
            switch(fdsi.ssi_signo) {    //判斷signal number
                ...
                case SIGCHLD:   //假設child結束
                    ...
                    printf(RED"real: "YELLOW"%ld.%ldsec\n", (endTime.tv_sec-startTime.tv_sec)+(endTime.tv_nsec - startTime.tv_nsec)/1000000000, 
                       ((endTime.tv_sec-startTime.tv_sec)*1000000000 + (endTime.tv_nsec - startTime.tv_nsec))%1000000000);
                    int HZ = sysconf(_SC_CLK_TCK);
                    printf(RED"user: "YELLOW"%04ld.%ld sec\n", fdsi.ssi_utime/HZ, fdsi.ssi_utime%HZ);
                    printf(RED"sys : "YELLOW"%04ld.%ld sec\n", fdsi.ssi_stime/HZ, fdsi.ssi_stime%HZ);
        ...
        if (strlen(cmdLine) <= 1)  //判斷長度是否大於1，判斷「使用者無聊按下enter鍵」
            continue;
        char *cmdPtr;
        cmdPtr = strtok(cmdLine, ";");
        while (cmdPtr != NULL) {
            char cmdCopy[256];
            strcpy(cmdCopy, cmdPtr);
            cmdPtr = strtok(NULL, ";");
            parseString(cmdCopy, &exeName);
